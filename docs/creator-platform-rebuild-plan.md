...
### 9.1 竞品基线（2026-08-23 更新）

对标后的共同门槛不是一次性生成，而是“可持续控制的创作生产”：

| 场景 | 创作者可感知的最低专业能力 | Operone 验收方向 |
|---|---|---|
| 一句话游戏 | 明确只支持可验证的窄 2D 类型；生成后能编辑规则/场景/行为；有可运行构建和脚本试玩证据 | 结构化 GameDesignSpec、Scene/Behavior 图、资产清单、浏览器 build 与首分钟回放/质量证据 |
| 一句话小说 | Story Bible、纲要、场景卡与正文相互可追溯；作者接受某一版本而非静默覆盖；连续性冲突可定位 | Bible/章节计划/章节正文都成为 artifact；续写前显示冲突与受影响章节；只将作者确认 revision 发布 |
| 小说转漫画 | 改编以已接受小说 revision 为输入；角色/服装/地点/画风锁定；分镜、对白、版式和每格重绘都可编辑与恢复 | Adaptation Bible、角色表和镜头计划版本化；面板记录参考、seed、渲染状态与失败原因；一致性/可读性阻断可解释 |

产品承诺修订：不宣传“任何一句话都能产出专业游戏/长篇/漫画”。页面应说明支持边界、生成所处阶段、失败能否恢复、以及下一步作者可操作的修改。

### 9.2 实施顺序

1. 完成 P1 已有 schema/耐久 worker 的读路径灰度与回退验证，禁止新增仅在内存运行的长任务。
2. 以小说作为端到端样板：将 Bible、纲要、场景和作者接受版本明确映射到 revision/artifact，并补连续性冲突的浏览器验收。
3. 漫画只接受已确认小说 revision，补跨章节角色/场景锁定和逐格重绘后的回归一致性验收。
4. 游戏从模板参数迁移到可编辑 Scene/Behavior 图，按窄类型提供脚本试玩/构建验收，不扩大未验证题材。
5. 外部条件具备后，启用受控真实模型演练、对象存储和支付；没有成本账本与限额不得开放批量生成。

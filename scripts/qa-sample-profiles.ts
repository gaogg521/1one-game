...
function main() {
  ...
  for (const s of SAMPLES) {
    ...
    if (s.id === "memory-match-mania") {
      assert(spec.samplePlayProfile?.puzzle?.memoryTimerSec === 90, "memory timer");
    }
    ...
    if (s.id === "tiny-planet-chopper") {
      assert(spec.samplePlayProfile?.shooter?.orbitChopper === true, "orbit chopper profile");
    }
    if (s.id === "grow-a-garden") {
      assert(spec.samplePlayProfile?.farming?.autoWater === true, "garden auto water");
    }
    if (s.id === "ultimate-3d-chess") {
      assert(spec.samplePlayProfile?.chess?.showLegalMoves === true, "chess legal moves");
    }
    if (s.id === "state-conquest") {
      assert(spec.samplePlayProfile?.strategy?.rushMode === true, "strategy rush");
    }
    if (s.id === "kids-puzzle") {
      assert(spec.samplePlayProfile?.puzzle?.kidsJigsaw === true, "kids jigsaw");
    }

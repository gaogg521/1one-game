...
项目把大模型分成 **三条链路**，部署时在 **`.env` / `.env.local`**（或 `docker-compose.yml`）里**显式写明**各变量；运行时解析集中在 **`src/lib/model-config.ts`**（`model-cascade.ts` 仅为兼容旧路径的 re-export）。

| 能力 | 默认主模型 | 默认备用 | 环境变量（网关上的 model id，支持 `litellm/` 前缀） |
|------|------------|----------|-----------------------------------------------------|
| **一句话生成游戏**（GameSpec、流式/多方案、规格修补、视觉参考解析等） | `gpt-5.2` | 仅当配置了 `OPENAI_MODEL_FALLBACKS` 时依次尝试 | `OPENAI_MODEL`、`OPENAI_MODEL_FALLBACKS`（**留空 = 无备用**，不再代码内偷偷注入） |
| **小说正文、漫画分镜 JSON** | `deepseek-v4-pro` | `doubao-seed-2-pro` | `NOVEL_LLM_PRIMARY`、`NOVEL_LLM_FALLBACK` |
| **小说/动漫等封面文生图** | `gpt-image-2` | `gemini-3.1-flash-image-preview`（需 `GEMINI_API_KEY`） | `IMAGE_GEN_OPENAI_MODEL`、`GEMINI_IMAGE_MODEL`、`IMAGE_GEN_SIZE` |

**文生图分辨率**：由 **`IMAGE_GEN_SIZE`** 控制（`1024x1024` / `1024x1536` / `1536x1024`），默认 **1024×1024（1K）**，见 `getImageGenDefaultSize()`。

**主要源码位置（进阶修改）**：

| 链路 | 模块 / 文件 |
|------|-------------|
| 统一解析（优先阅读） | **`src/lib/model-config.ts`** |
| 游戏 / 规格（OpenAI 兼容串联） | `getModelCascade()`；调用方含 `src/lib/generate-spec.ts`、`src/lib/multi-agent-spec.ts`、`src/app/api/generate/patch/route.ts`、`src/lib/vision-reference.ts`、`src/lib/llm/models.ts` |
| 小说 / 漫画文案 | `getNovelStyleTextModelCascade()`；配置见 **`src/lib/novel-generate-config.ts`**；`src/app/api/novel/generate/route.ts`、`src/app/api/novel/generate/stream/route.ts`、`src/app/api/comic/generate/route.ts` |
| 文生图 | `src/lib/image-generation.ts`（读 `model-config`）；封面编排见 `src/lib/cover-generation.ts` 等 |

Docker 部署可参考仓库根目录 **`docker-compose.yml`** 中与 `OPENAI_*`、`NOVEL_LLM_*` 相关的环境变量默认值。

---

## 环境变量

复制 `.env.example` 为 `.env` 后按需填写。常用项如下（**模型主备与三类链路说明见上一节 [模型配置](#模型配置)**）：
...
| `OPENAI_MODEL` | **游戏 / 规格链路**主模型 ID（默认 `gpt-5.2`） |
| `OPENAI_MODEL_FALLBACKS` | **游戏 / 规格链路**备用模型，逗号分隔；**留空表示不设备用**（不再使用代码内隐式默认） |
| `NOVEL_LLM_PRIMARY` | **小说 / 漫画 JSON** 主模型（默认 `deepseek-v4-pro`） |
| `NOVEL_LLM_FALLBACK` | **小说 / 漫画 JSON** 备用模型（默认 `doubao-seed-2-pro`） |
| `NOVEL_LLM_TIMEOUT_MS` | **小说正文**单次 LLM 超时（毫秒，默认 **`600000`**）；流式与非流式共用 |
| `NOVEL_LLM_MAX_OUTPUT_TOKENS` | **小说正文**最大输出 token（默认 **`65536`**，仍受网关/模型实际上限约束） |
| `NOVEL_MIN_ACCEPT_CHARS` | **小说正文**认为成功的最短字数下限（默认 `900`，代码不低于 `400`） |
| `LLM_WITH_TIMEOUT_MAX_MS` | 兼容网关请求的 Promise 等待硬顶（毫秒，默认 `600000`，须 ≥ 小说超时） |
| `IMAGE_GEN_OPENAI_MODEL` | **文生图** OpenAI 侧模型（默认 `gpt-image-2`） |
| `GEMINI_IMAGE_MODEL` | **文生图** Gemini 侧模型 ID（默认 `gemini-3.1-flash-image-preview`） |
| `IMAGE_GEN_SIZE` | **文生图**默认边长：`1024x1024`（默认） / `1024x1536` / `1536x1024` |
| `GEMINI_API_KEY` | 走 Gemini 文生图备选路径时需要 |
...
| `POST` | `/api/novel/generate/stream` | 小说正文 **SSE**（`text/event-stream`）；事件含 `delta` 增量、`done` 携带完整 `novel` |
...
| `npm run qa:template-matrix` | 六模板离线矩阵：`mockSpecFromPrompt` + director 字段（无需启动服务） |
| `npm run simulate:handtest` | HTTP 手测模拟：创建项目、试玩、refine 摘要等（需 **`npm run start`** 在 8888；见 `scripts/run-handtest-all.ps1` 一键串联） |
| `npm run test:e2e` | Playwright：默认走 **`npm run dev`（8888）**；本地生产形态可先 **`npm run build`** 再运行 **`PW_START=1 npm run test:e2e`**（等价于 **`next start` + PORT=8888**，与 CI 一致）；refinement 手测加 **`E2E_REFINE_STUB=1`**，已有服务时 **`PW_EXTERNAL=1`** |

首次本机跑 E2E 需 **`npx playwright install chromium`**（CI 已由 workflow 安装）。

**请求侧可观测**：生成类路由在 **`GENERATE_STRUCTURED_LOG=1`** 时向 stdout 打一行的 JSON（含 `requestId`、耗时、条目数摘要等）。**可选**：**`GAME_SPEC_JSON_SCHEMA_INCLUDE_DIRECTOR=1`** 让游戏规格 LLM 的 strict schema 包含可选 **director**（部分网关可能不兼容，默认关闭）；**`VARIANTS_DIRECTOR_SUMMARY=1`**（且非 production）为 variants 响应附带轻量 **directorSummary**。

**单机限流**（`/api/generate*`、`/api/projects/[id]/refine`）：按 **Cookie/ownerKey + IP** 在进程内计数（`GENERATE_RL_POST_MAX`、`GENERATE_RL_STREAM_MAX`、`GENERATE_RL_VARIANTS_MAX`、**`GENERATE_RL_REFINE_MAX`**、`GENERATE_RL_WINDOW_MS`）；多副本或无状态时请改用 **网关 / Redis**，勿依赖单机内存窗口。
...
The repo uses **three model pipelines**. Set variables in `.env` / `.env.local` (or compose); runtime parsing lives in **`src/lib/model-config.ts`** (`model-cascade.ts` re-exports for legacy imports).

| Capability | Default primary | Fallback when configured | Env vars |
|------------|-----------------|--------------------------|----------|
| **Game / GameSpec** (generate, stream, variants, patch, vision refs) | `gpt-5.2` | Ordered list from `OPENAI_MODEL_FALLBACKS` only | `OPENAI_MODEL`, `OPENAI_MODEL_FALLBACKS` (**empty = no fallbacks**, no hidden injection) |
| **Novel text & comic panel JSON** | `deepseek-v4-pro` | `doubao-seed-2-pro` | `NOVEL_LLM_PRIMARY`, `NOVEL_LLM_FALLBACK` |
| **Cover / text-to-image** | `gpt-image-2` | `gemini-3.1-flash-image-preview` (needs `GEMINI_API_KEY`) | `IMAGE_GEN_OPENAI_MODEL`, `GEMINI_IMAGE_MODEL`, `IMAGE_GEN_SIZE` |

**Image size**: **`IMAGE_GEN_SIZE`** — `1024x1024` (default 1K) \| `1024x1536` \| `1536x1024` (`getImageGenDefaultSize()`).

**Where to change code**:

| Pipeline | Files |
|----------|-------|
| Central parsing | **`src/lib/model-config.ts`** |
| Game / spec cascade | `getModelCascade()`; callers include `src/lib/generate-spec.ts`, `src/lib/multi-agent-spec.ts`, `src/app/api/generate/patch/route.ts`, `src/lib/vision-reference.ts`, `src/lib/llm/models.ts` |
| Novel / comic text | `getNovelStyleTextModelCascade()`, **`src/lib/novel-generate-config.ts`**, `src/app/api/novel/generate/route.ts`, `src/app/api/novel/generate/stream/route.ts`, `src/app/api/comic/generate/route.ts` |
| Image generation | `src/lib/image-generation.ts` (+ cover helpers like `src/lib/cover-generation.ts`) |

See also **`docker-compose.yml`** for compose defaults.

### Environment variables

See `.env.example`. Besides `DATABASE_URL`, `OPENAI_API_KEY`, `OPENAI_BASE_URL`, optional `OPENAI_EXTRA_HEADERS_JSON`, `OPENAI_USER_AGENT`:

| Variable | Purpose |
|----------|---------|
| `OPENAI_MODEL` | Game/spec primary model (default `gpt-5.2`) |
| `OPENAI_MODEL_FALLBACKS` | Game/spec fallbacks, comma-separated (**empty = none**, no code-side implicit default) |
| `NOVEL_LLM_PRIMARY` | Novel + comic JSON primary (default `deepseek-v4-pro`) |
| `NOVEL_LLM_FALLBACK` | Novel + comic JSON fallback (default `doubao-seed-2-pro`) |
| `NOVEL_LLM_TIMEOUT_MS` | Novel LLM timeout in ms (default **`600000`**); shared by stream + non-stream |
| `NOVEL_LLM_MAX_OUTPUT_TOKENS` | Novel max output tokens (default **`65536`**, still subject to gateway/model caps) |
| `NOVEL_MIN_ACCEPT_CHARS` | Min accepted novel length in chars (default `900`, floored at `400` in code) |
| `LLM_WITH_TIMEOUT_MAX_MS` | Hard cap for LLM `Promise.race` waits in ms (default `600000`; should cover novel timeout) |
| `IMAGE_GEN_OPENAI_MODEL` | Cover image OpenAI model (default `gpt-image-2`) |
| `GEMINI_IMAGE_MODEL` | Cover image Gemini model id (default `gemini-3.1-flash-image-preview`) |
| `IMAGE_GEN_SIZE` | Default output size: `1024x1024` (default) \| `1024x1536` \| `1536x1024` |
| `GEMINI_API_KEY` | Required for Gemini image fallback |

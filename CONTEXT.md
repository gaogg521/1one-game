...
- 未实现统一创作台的质量报告展示、小说逐章/漫画逐页的交互式定向修复入口；当前先提供详情与生成 API 的确定性质量摘要。
...
1. 在统一创作台消费 API 的质量报告，展示证据并提供“只修当前章节/当前页/当前资产”的明确入口。
2. 将小说评分细化到章、漫画评分细化到页，并将真实阅读/试玩数据写进质量报告；先观察阈值，不直接阻断发布。
3. 完成五个旗舰模板的运行时角色状态机、资产制作和真实 60 秒 E2E 后，再把质量门禁分阶段启用。

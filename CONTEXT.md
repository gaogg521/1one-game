...
### 状态
- 已 commit `d64c9898` 并部署生产 ✅

### 服务器迁移（同会话续）
- `docs/server-migration.md` — AI/运维迁移手册
- `scripts/prod_ssh.py` — `OPERONE_DEPLOY_*` 环境变量
- `scripts/backup-prod-for-migration.py` / `restore-prod-migration.py`

---

## Session 51 · 斗地主头像修复 + 音效 + 点击精度（2026-06-22）

### 完成内容
1. **背景音乐**：叫地主阶段结束后 800ms 调用 `soundscape.setTension(0.4)` 激活 BGM
2. **出牌音效**：`commitPlay()` 按牌型（炸弹→boss/explode、飞机→levelUp、顺子→power 等）播放不同音效 + 粒子
3. **出牌点击精度**：非末张牌 hitW 缩为 `spacing`（≤32px），消除重叠区点击错位
4. **SFX 包**：douDizhu/mahjong 改用 `shuffle` 包（`createPhaserGame.ts`）
5. **头像统一**：
   - seat 0（玩家）改为 SVG 圆框头像（同 AI），上移 44px 确保出现在牌桌椭圆内
   - 三个座位角色互换：seat 0 = girl，seat 1 = man，seat 2 = girl
   - 移除 `drawAvatar` / `AVATAR_COLORS` 导入
6. **头像内人物缩小 80%**：GIRL_HEAD_SVG + MAN_HEAD_SVG 内部 `<g>` 加 `transform scale(0.80)`，外圆框不变

### Commits
- 音效/点击精度：`c86ef127`（三席 TS 补全顺带）、此前本会话改动
- 头像：`edef343e`

### 状态
- TypeScript 零错误 ✅，预览截图确认三个头像统一 ✅
...
## Session 52 · 斗地主 SVG 角色 → 头像、布局修复、HudFrame 计时器修复
...
| 文件 | 改动摘要 |
|------|----------|
| `src/game/engine/avatar-draw.ts` | 新增 `GIRL_SVG`/`MAN_SVG` inline SVG 常量（viewBox 100×150，2× 渲染）；新增 `preloadQqCharacterTextures(scene)`；`drawQqCharacter()` 改为优先返回 `Image`（SVG 纹理），Graphics 回退 |
| `src/game/engine/DouDizhuScene.ts` | 底牌容器 y 从 `viewH*0.14`→`viewH*0.27`（修复与叫地主 banner 重叠）；AI 头像改回 `drawAvatar()` 小圆头像（不再用全身 SVG）；底牌 depth 20→190（最前层）；`update()` 加 `hud.update({})` 驱动 goal/banner 计时器；`create()` 100ms 后 `hud.dismissGoal()` 关闭通用 Goal 弹窗 |
| `src/game/engine/HudFrame.ts` | 无改动（bug 在 DouDizhuScene，其他 16 个场景已有每帧 hud.update）|

### 关键技术决策
- **SVG inline 方案**：`data:image/svg+xml,${encodeURIComponent(svg)}` 传给 `scene.load.image()`，Phaser 在 `preload()` 同步加载，`create()` 中可用；2× 尺寸渲染后 `setScale(0.5)` 避免模糊
- **斗地主不用全身角色**：全身角色适合需要主角感的场景，斗地主桌游用小圆头像更紧凑合理
- **Goal 弹窗问题根因**：`hud.update()` 只在事件驱动的 `refreshHud()` 里调，导致 goal/banner 淡出计时器不执行；修复为在 scene `update()` 每帧调用
- **其他 16 个 HudFrame 场景**：均已在各自 `update()` 里调 `hud.update({})`，不受影响
...
2. 打开 `/en/play/sample-dou-dizhu`，确认：底牌（Kitty）3 张清晰可见在最前层、小圆头像、Goal 弹窗快速消失、叫地主横幅无遮挡
3. `avatar-draw.ts` 中的 SVG 字符串可用于其他需要全身角色的场景（麻将、棋类等）

---

## Session 51 · QQ 斗地主全身角色 + 修复 sceneGame i18n 丢失

### 完成内容
1. **avatar-draw.ts 重构**：新增 `drawQqCharacter(scene, cx, cy, type, opts)`
   - `type = "girl"` — 粉裙马尾女生，白高光眼，红鞋红腮，Phaser Graphics 纯绘制
   - `type = "man"` — 白袍白帽，大胡子，皮带深鞋
   - 响应式 `opts.scale`，统一锚点为 footY（人物底部）
2. **DouDizhuScene.ts** `buildAvatars()`：
   - 左 AI（seat 1）→ `drawQqCharacter(..., "girl")` 在 `viewW * 0.08`
   - 右 AI（seat 2）→ `drawQqCharacter(..., "man")` 在 `viewW * 0.92`
   - 玩家席（seat 0）保持原小头像
3. **修复 sceneGame i18n 大规模丢失**：
   - 根因：commit `d64c9898`（mobile feed v2）整体覆盖了 5 个 locale 的 messages 文件，导致 `sceneGame.{douDizhu,chess,card}` 顶级对象全部消失
   - 修复：从 `d1845548` 还原全部 3 子对象，注入回 5 个 locale 文件
   - 验证：截图确认`叫地主`、`底牌`、`右家`、`农民`、`大/小`等 key 正确翻译 ✅
4. **TypeScript `tsc --noEmit` 零错误** ✅
5. **commit `0d43d897` + push** ✅

### 下次启动清单
1. `npm run dev`（port 8888）
2. 打开斗地主游戏，确认左侧粉裙女生 + 右侧白袍大叔出现在桌旁
3. 叫地主阶段确认 UI 文字均为中文（无 raw key 显示）
4. 打开英文版 (`/en/play/…`)，确认 Joker "Sm"/"Lg"、"Bid 1/2/3/Pass" 正常

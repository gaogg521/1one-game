...
## 当前状态
- 「当前线上生效」原先只画全局底稿，locale 覆盖被折叠，管理员会误判中文站模型。已改为中文站/国际站两列最终命中。
- 测试作品已是 `public+ready`：小说/漫画发现按最新能看到；游戏样品馆是精选 sample-*，不含测试游戏。游戏发现默认按游玩数，新游会被压下去。
- 统一入口：`/console?tab=works` 作品治理；待审 `/console?tab=pending`；样品馆 `/console?tab=samples`。

## 下次启动清单
1. 部署本轮 Console 生效表改动。
2. 打开 `/console?tab=runtime` 应看到中文列 `openrouter/free`（小说/文生图）与全局 DeepSeek/Seedream 不一致。
3. 打开 `/console?tab=works` 搜「残谱」「月种」。
...
## P56：Astrocade 首页十款玩法的 clean-room 样品实现（2026-08-28）

- 以 2026-08-28 实际浏览到的 Astrocade 首页前十款游戏为行为参考，新增十个自主命名、无第三方源码/素材/商标复制的可完成样品：方块建造、圈地、百层击破、庄园合成、方块狙击、昼夜生存、载客列车、英雄融合、洗车流程、双队射击。
...
## P60：随机竞品 clean-room 生产复测与首玩延迟解耦（2026-08-30）
...
- 第三个项目最终 production candidate 98/100，但公网启动真实报错 `player.setCollideWorldBounds is not a function`，证明 mock runnable 门禁存在假阳性。AgenticScene 现为 `physics.add.existing` 的普通形状补齐常用 Arcade body 链式代理，并捕获 `create` 异常，不再永久卡加载；部署后需用同一项目重载实测。
...
### 体验修正（2026-09-11）：自动修复期间不再误显终态失败

- 实测项目“开心消消乐”（`cmtwtjyli00on4abzhpx6xqgl`）前两版分别因素材与重启状态问题失败，后台已自动进入下一次修复，第三版通过；旧页面却优先读取失败 revision，导致自动修复期间展示红色“游戏未通过运行验证”。
- 页面状态现以活动 job 为准：`queued` / `running` / `retrying` 一律展示制作或自动优化进度，只有任务结束且仍有致命运行故障时才展示柔和的继续优化入口。
- 最终硬拦截收缩为真实不可玩证据：独立运行时缺失或验证缺失、运行报错、未启动、无首帧、循环卡死、交互无变化、主角不可见或主角不响应操作。
- Forge 静态质量发现、必需素材缺失但已有运行时占位图、收集物不可见或越界仍会进入自动修复与证据记录；在修复预算用完后不再单独阻止一个已经通过真实 iframe 启动和交互验证的游戏交付。
- 修复了运行时验证证据被最终探针摘要覆盖的问题，素材与静态 advisory 现在会保留在 validation evidence 中。
- 本地验证：`qa:runtime-delivery-gate`（正常、语法错误、启动报错、空壳，以及软硬玩家发现分类）、`tsc --noEmit`、目标 ESLint（0 error）。
...
- `bc0dd335` 已通过生产发布门禁；重验保留健康样例passed，并将第8版明确置为failed（`runtime_entity_discriminator_mismatch`）。同项目自动修复到revision `cmtvktkm00038yj4g0lvc7739`：首屏、键盘双向、触屏目标0.70/实际0.6994均通过；真实胜利70.11秒/240分并重开，真实失败16.75秒/50分并重开。
- 但人工查看两张重开截图发现角色没有回到初始中点：`player_control.targetX`、spawn timers和collision cooldowns是模块级闭包变量，SDK `restart()`虽清空world/state，却不会重建模块闭包，旧局输入/节奏污染新局。新增 `restart_state_not_reset` 硬门禁：system模块中会被修改的target/timer/cooldown/accumulator/count/wave等重开敏感顶层状态不得留在闭包，必须放入由init/restart重建的G状态；修复路由覆盖相关system和main，代码提示词同步。胜利验收脚本另修复实体过多时player evidence被采样上限挤掉导致使用旧坐标的问题，改为从真实player图片绘制读取当前屏幕位置。`qa-game-forge`、TypeScript、目标ESLint均exit=0；当前revision暂不发布，待部署重验并让同一项目生成下一版。
- `0e2dd716` 生产发布成功，但首次重验误伤旧健康样例：它的 `spawnTimer` 虽是闭包变量，却由 `G.clearItems()` 重置且main的init/restart真实调用该hook；HUD的 `hintTimer` 只影响装饰提示。规则已收窄：允许由main实际调用的reset/clear/init hook重置私有状态，排除HUD/render/audio/fx装饰状态；新增合法reset hook回归，修复路由只改finding消息中点名的模块与main。回归、TypeScript、ESLint均exit=0；需重新部署并确认旧样例恢复passed、星港接星仍failed。
...
## 2026-09-12 · 手机质量合同、可玩性检测与美术管线（本会话）

### 当前状态

- 编译：`npx tsc --noEmit` 通过；`npm run build` 通过；目标 ESLint 0 error。
- 生产分支 `main` 头部 `a42bfde8`，已部署。
- 新增/修复的 QA：`qa:mobile-quality-contract`、`qa:game-player-visibility`（后者此前未注册且断言的是门禁软硬拆分前的旧行为，一直失败无人发现）。

### 本会话交付（按提交顺序）

| 提交 | 内容 |
|---|---|
| `4d33820f` | 手机交付成为平台级生成合同（竖屏、主角尺寸下限、首 60 秒难度包络写进设计与模块提示词） |
| `207cc2cf` | 已交付但未达标的构建自动打磨一轮（advisory 驱动，不加硬门禁） |
| `42876d4f` | 主角尺寸改为真实测量（渲染器本就算了，之前被丢弃） |
| `6d97bf7b` | 美术审查接真实交付帧（此前 `visual_review_agent` 无生产者） |
| `dee360f2` | 美术审查失败必须说明原因（脱敏日志） |
| `4c0b0bba` | 素材缺失时不再命令运行时绘制 404 素材 |
| `2ca88bb2` | 必需素材失败做一次窄重试（`missingRequiredSlots` 此前算了无人消费） |
| `0f4903ca` | 失败的自动优化轮不得让可玩游戏下线（回滚到最后通过的构建） |
| `aa32ab36` | 交付判定必须描述**正在服务**的那个构建 |
| `e8fd4a1d` + `a42bfde8` | 创作者写明的颜色被尊重（生成与保存两条路径） |
| `a8ed4077` | **核心循环收敛检测**：探针横扫 12 秒读心跳分数 |
| `c602663a` | 竖屏默认 540×1170；门禁 iframe 由 393×700 改为真机 393×852 |
| `7b44d2f4` | **静态检查：读了没人 spawn 的实体组** |
| `d13a2553` | **静态检查：score/lives 只存在私有对象** |
| `66329946` | 确定性失败（缺密钥/401）不重试 |
| `3d2e85eb` | **精灵生成超时从 75 秒提到 3 分钟**（根因，见下） |
| `7f2108e8` | 放置类游戏不再被"主角不可见"硬门禁拒绝 |
| `876a3358` | **不再生成第二套游戏用不到的美术**（根因，见下） |

### 两个根因级发现

**1. 生成的游戏普遍没有角色美术 —— 是超时，不是密钥。**
日志显示 `未配置 GEMINI_API_KEY`，把人引向错误方向。真实原因：漫画线调图片生成不传超时、拿到 12 分钟默认值；游戏精灵传 `artSlotTimeoutMs = 75_000`，每个槽位精确在 75001ms 被**本平台自己的 AbortSignal** 掐断，异常又被 `catch {}` 吞成通用信息，再拼上回退链末尾的 Gemini 提示。同一个模型、同一个网关、同一份密钥，漫画能出图游戏不能，唯一差别是预算。提到 3 分钟后，植物大战僵尸 5 个素材全部成功。

**2. 生成一次游戏要 15 分钟 —— 其中 13 分钟在做无用功。**
作业里程碑：设计 21 秒、7 个模块 53 秒、5 个素材 121 秒。之后 worker 又跑旧模板素材管线 `runProjectAssetPipeline`，生成 Forge 运行时根本不读的通用 player/hazard/gem 精灵，期间不上报进度（卡在 `percent: 8`）。更糟：旧背景写在 `/game-bg/{projectId}.png`，**会覆盖 Forge 刚生成的游戏自己的背景**。有 forgeBuild 时已跳过（封面仍保留）。

### 质量门的三类盲区（本会话补上前两类）

平台的检查此前只回答"它动不动"：启动了、有首帧、循环没卡死、主角可见、主角响应输入、像素有变化。于是以下缺陷全部静默通过：

1. **核心循环不收敛**（`a8ed4077`）：真机试玩 70 秒分数恒为 0、计时归零不结算，门禁判 `passed` 评分 98。
2. **跨模块名字不一致**（`7b44d2f4` / `d13a2553`）：碰撞遍历 `'enemy'` 而生成方是 `'meteor'`；或分数只记进私有 `G.state`，SDK 计数器从没被碰过、`g.state.time` 从不推进导致超时分支是死代码。两者运行时都不报错。
3. **门禁编码单一品类**（`7f2108e8`）：硬门禁假设永远有常驻主角，塔防/种田/建造类结构性无法交付——植物大战僵尸跑满 10 分钟后因"开局场上没有豌豆射手"被拒。

### 生产实测

- 太空快递 `cmtwv5zla000e8w6tkzc8jmmt`：屏幕填充 82%→100%、从不结算→有结算卡、`ship_blue.png` 404→200、灰紫主角→亮蓝。**组名 blocker 在我未告知组名的情况下驱动 Agent 自己修好了碰撞**，而此前两轮人工精确指令都没改到那个名字。
- 植物大战僵尸 `cmtxzvaqx00078c63j9xem3br`：seq1/seq2 均 ready，Forge 耗时 2 分 13 秒，5 个素材全部生成。真机试玩：种植可用、僵尸推进、阳光累计、结算卡正常、音效有声、填充 100%。**遗留**：Forge QA 有一条未修掉的 `undeclared_shared_state`（`main_game` 读 `G.zombieKills` 但无人提供），导致"击退 12 只僵尸"的胜利条件永远不成立，实测得分恒为 0。

### 已知待办

- [ ] `G.zombieKills` 这类"读了没人提供的共享状态"已有检查（`undeclared_shared_state`），但 Forge 修复轮两次未修好即放行。应考虑把它并入交付前必修，或提高修复轮对该码的针对性。
- [ ] 视觉审查仍不稳定：120 秒仍会超时（成功过，但非稳定）。不要再盲目加时间，先确认是"慢"还是"卡住"。
- [ ] 部署脚本原地重建 `.next`，每次发布有真实全站 500 窗口（首页也 500，`/api/health` 仍 200）。应构建到临时目录再原子切换。
- [ ] 质量发现对创作者与后台仍不可见（`game_production_candidate.advisories` 未进 `creator-quality.ts` 与后台）。

### 下次启动清单

1. 读本节与 `HANDOFF.md` 同日章节。
2. `python scripts/check-prod-commit.py` 确认生产 commit 与 `main` 一致。
3. 要判可玩性用 `npx tsx scripts/playtest-local-runtime.ts <url> <秒> <输出目录>`；**不要**用 Claude 浏览器面板下结论——窗口被遮挡时 rAF 被节流到约 1/5 速（28 秒真实时间只走 5 游戏秒）。
4. 生成新游戏用 `scripts/create-prod-game.ts`（只创建与等待，不发布）；定向修改用 `scripts/refine-prod-game.ts`。
5. `scripts/playtest-local-runtime.ts` 的 `addInitScript` 必须传字符串：内联箭头函数会被 tsx/esbuild 注入 `__name`，页面里不存在，整段探针静默失败返回空。

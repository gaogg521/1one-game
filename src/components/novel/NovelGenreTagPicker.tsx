...
import { getLocalizedNovelGenreTag } from "@/lib/novel-genre-tags";
...
/** 网文类型标签单选 */
export function NovelGenreTagPicker({ value, onChange, disabled, locale = "zh-Hans" }: Props) {
  return (
    <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
      {NOVEL_GENRE_TAGS.map((tag) => {
        const selected = value === tag.id;
        const localized = getLocalizedNovelGenreTag(tag, locale);
        return (
          <button
            key={tag.id}
            type="button"
          ...
          >
            <span className="block text-sm font-semibold text-[var(--gc-text)]">{localized.label}</span>
            <span className="mt-0.5 block text-xs leading-relaxed text-[var(--gc-muted)]">{localized.desc}</span>
          </button>
        );
      })}
    </div>

...
function createGame(ctx, Phaser) {
  return { create(scene) {
    ...
    const shade=scene.add.graphics().setDepth(-10);shade.fillStyle(0x07111f,.20);shade.fillRect(0,0,w,h);shade.fillStyle(0xf59e0b,.16);shade.fillCircle(w*.5,h*.22,w*.24);
    const world=scene.add.graphics().setDepth(-7);world.fillStyle(0x10281f,.96);world.fillRect(0,h*.56,w,h*.44);world.fillStyle(0x183b2c,.95);for(let i=0;i<8;i++){const x=(i+.25)*w/8;world.fillTriangle(x-32,h*.58,x+32,h*.58,x,h*(.36+(i%3)*.035));}world.fillStyle(0x2d1f16,.9);world.fillRect(w*.42,h*.29,w*.16,h*.12);world.fillStyle(0xd69e52,.85);world.fillTriangle(w*.39,h*.3,w*.61,h*.3,w*.5,h*.22);
    const road=scene.add.graphics().setDepth(-5);road.fillStyle(0x252b36,.98);road.fillTriangle(w*.025,h,w*.975,h,w*.59,h*.29);road.fillTriangle(w*.025,h,w*.59,h*.29,w*.41,h*.29);road.lineStyle(5,0xf8e8c5,.95);road.lineBetween(w*.025,h,w*.41,h*.29);road.lineBetween(w*.975,h,w*.59,h*.29);road.lineStyle(3,0xf6c453,.9);road.lineBetween(w*.35,h,w*.47,h*.29);road.lineBetween(w*.65,h,w*.53,h*.29);road.lineStyle(1,0xffffff,.13);for(let y=h*.35;y<h;y+=38)road.lineBetween(w*.09+(h-y)*.42,y,w*.91-(h-y)*.42,y);
    for(let i=0;i<5;i++){const y=h*(.42+i*.115),spread=.13+i*.09,size=12+i*5;for(const side of [-1,1]){const x=w*(.5+side*spread);scene.add.rectangle(x,y,size*.38,size*2.5,0x6b3d1f).setDepth(-3);scene.add.circle(x,y-size*1.3,size,0x1f6b3b,.96).setDepth(-3);scene.add.circle(x-side*size*.55,y-size*.8,size*.7,0x2f8550,.9).setDepth(-3);}}
    let lane=1,targetX=lanes[1],vy=0,score=0,coins=0,lives=3,speed=250,elapsed=0,ended=false,lastSpawn=0,lastHit=-999;
    const heroW=Math.max(88,w*.15),heroH=Math.max(122,h*.16);let player;if(ctx.assets?.playerKey) player=scene.add.sprite(targetX,ground,ctx.assets.playerKey).setDisplaySize(heroW,heroH).setDepth(8);else player=scene.add.rectangle(targetX,ground,heroW*.7,heroH*.86,0xf59e0b).setDepth(8);
    const shadow=scene.add.ellipse(targetX,ground+heroH*.44,heroW*.9,Math.max(18,heroH*.14),0x000000,.42).setDepth(7),actors=[];
    ...
    function spawn(kind,laneIndex){const key=kind==='coin'?ctx.assets?.collectibleKey:ctx.assets?.enemyKey;let obj;if(key)obj=scene.add.sprite(lanes[laneIndex],h*.3,key).setDisplaySize(kind==='coin'?42:78,kind==='coin'?42:86);else obj=kind==='coin'?scene.add.circle(lanes[laneIndex],h*.3,17,0xfacc15):scene.add.rectangle(lanes[laneIndex],h*.3,64,76,0xef4444);obj.setDepth(5);actors.push({obj,kind,lane:laneIndex,z:0});}
    scene.events.on('update',(_time,delta)=>{if(ended)return;const dt=Math.min(delta,40)/1000;elapsed+=dt;speed=Math.min(520,speed+dt*7);score+=dt*speed*.08;player.x=Phaser.Math.Linear(player.x,targetX,Math.min(1,dt*13));shadow.x=player.x;vy+=1180*dt;player.y=Math.min(ground,player.y+vy*dt);if(player.y>=ground)vy=0;shadow.alpha=(player.y<ground-20)?.18:.35;
      ...
      for(let i=actors.length-1;i>=0;i--){const a=actors[i];a.z+=dt*speed/410;const scale=.38+a.z*1.22;a.obj.y=h*.29+a.z*(ground-h*.29);a.obj.x=Phaser.Math.Linear(w/2,lanes[a.lane],Math.min(1,a.z));a.obj.setScale(scale);if(a.z>.82&&a.z<1.08&&a.lane===lane&&Math.abs(player.y-ground)<42){if(a.kind==='coin'){coins++;score+=25;ctx.onScore(1);a.obj.destroy();actors.splice(i,1);}else if(elapsed-lastHit>33){lastHit=elapsed;lives--;scene.cameras.main.flash(90,255,90,45,.25);scene.cameras.main.shake(150,.012);a.obj.destroy();actors.splice(i,1);if(lives<=0){ended=true;ctx.onEnd(false);}}}else if(a.z>1.15){a.obj.destroy();actors.splice(i,1);}}
      hud.setText(Math.floor(score)+' m  ·  ◆ '+coins+'  ·  ♥ '+lives);if(elapsed>=65){ended=true;ctx.onEnd(true);}
    });
  }};
}`;

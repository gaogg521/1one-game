...
function createGame(ctx, Phaser) {
  return { create(scene) {
    ...
    let player;if(ctx.assets?.playerKey) player=scene.add.sprite(targetX,ground,ctx.assets.playerKey).setDisplaySize(88,122).setDepth(8);else player=scene.add.rectangle(targetX,ground,62,104,0xf59e0b).setDepth(8);
    const shadow=scene.add.ellipse(targetX,ground+55,82,18,0x000000,.42).setDepth(7),actors=[];

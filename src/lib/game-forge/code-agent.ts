...
One module owns all HUD fields: put score, lives and timer in ONE g.ui.hud call. Do not draw a second timer or title over the HUD's top 60 pixels. Draw the background first and the player afterwards at a clearly visible size; collision geometry must match the visible sprite. A touch drag must move the same player entity as keyboard input. Restart must recreate that entity and reset every timer, score and spawn accumulator. Restart-sensitive state such as targets, cooldowns, spawn timers, counters, waves and elapsed time MUST live on a G state object that init/restart recreates; never keep that state in module-level var/let closures because those closures survive replay.

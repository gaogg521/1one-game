...
export const NOVEL_BRIEF_PACKS: NovelBriefPack[] = [
  ...
  },
  {
    ...
    label: "儿童短篇",
    logline: ({ userLine }) =>
      `家长输入：「${userLine.slice(0, 80)}」。按读者档位改编为轻量童趣短故事。`,
    setting: "单一场景（如春日山野、草地），由输入推导",
    world: "温暖安全、童真童趣；典故只留善意探索，低幼档不讲史料",
    protagonist: "原创主角（小朋友或小动物，须起名）",
    characters: ["一个可爱动物伙伴"],
    antagonists: [],
    coreConflict: "无强冲突：好奇观察、温柔发现",
    protagonistGoal: "和伙伴一起感受典故里的善意与探索乐趣",
    plotBeats: [
      "听故事启发，和伙伴去看花草",
      "观察摸一摸，感受为别人动脑筋",
      "明白不私自尝野草，要问大人",
    ],
    keyScenes: ["山野草地边看花草", "伙伴一起开心笑"],
    tone: "温馨童趣·轻快治愈",
    writingStyle: ["短句顺口", "拟声词与可爱动作", "不重不吓"],
    narrativeHints: [
      "一条线、单场景",
      "仅主角+1伙伴，禁止老人导师",
      "禁止肚子痛、吃苦、放弃、尝野草",

...
- version 固定为 1
- templateId 只能是 avoider | collector | survivor | platformer | towerDefense
  ...
  · towerDefense：塔防（路径行军 + 塔位建造升级 + 波次）；winScore 表示总波次数（建议 6–12）；baseHealth 表示基地生命（约 24–80）；startingCoins 开局金币（约 80–220）；hazardSpeed 影响敌军沿路径移动快慢；spawnIntervalMs 同波内出兵间隔；playerSpeed 映射炮塔射速；labels.collectible 填金币/能量等货币称呼
...
示例（语气参考，勿照抄内容）：
{"version":1,"templateId":"collector","title":"霓虹港区速递","theme":{"backgroundColor":"#0f172a","playerColor":"#38bdf8","hazardColor":"#f43f5e","collectibleColor":"#fbbf24","particleTint":"#a78bfa"},"gameplay":{"playerSpeed":320,"hazardSpeed":210,"spawnIntervalMs":720,"winScore":28,"lives":4,"arenaPadding":36,"jumpStrength":420,"gravity":980,"startingCoins":120,"baseHealth":48},"labels":{"player":"无人机","hazard":"广告牌碎片","collectible":"包裹","subtitle":"夜色里穿行送货"}}

若描述含糊：默认 avoider；若强调塔防/保卫萝卜/波次/箭塔/防线→towerDefense；若强调横版跳跃/平台闯关/多层地形→platformer；若强调收集/金币/宝石→collector；强调生存/扣血/多条命→survivor。`;
...
const JSON_SCHEMA = {
  ...
  schema: {
    ...
    properties: {
      ...
      templateId: {
        type: "string",
        enum: ["avoider", "collector", "survivor", "platformer", "towerDefense"],
      },
      title: { type: "string" },
      theme: {
        type: "object",
...
export async function generateGameSpecDraftWithMeta(
  ...
  if (options?.searchEnhance) {
    const res = await tryWebEnhance(clean);
    augmented = res.prompt;
    web = res.meta;
  }
  ...
  const fromLlm = await withTimeout(
...
export async function enhanceGameSpecFromDraftWithMeta(params: {
  ...
  options?: Pick<GenerateOptions, "templateHint">;
}): Promise<{ spec: GameSpec; source: GenerationSource; web?: WebEnhanceMeta | null; debug: GenerationDebug }> {
  ...
  try {
    ...
    for (const r of picked) {
      try {
        ...
        pages.push({ title: title ?? r.title, url: r.url, text, score: r.score });
      } catch {
        // 单条失败跳过
      }
    }
...
export async function generateGameSpecWithMeta(
  ...
  const draft = await generateGameSpecDraftWithMeta(prompt, options);
  if (!enhancedRequested) return draft;
  // 若初稿已经回退到 mock，则跳过二次强化（避免无意义等待超时）。
  ...
  const enhanced = await enhanceGameSpecFromDraftWithMeta({
    ...
    web: draft.web ?? null,
    options: { templateHint: options?.templateHint },
  });
  return enhanced;
}

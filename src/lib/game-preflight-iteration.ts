...
export function selectGameQualityPolishFindings(advisories: readonly string[]): string[] {
  const codes = advisories.map((entry) => {
    ...
    // Some advisories carry evidence after a colon (which HUD fields, which
    // asset). Match the code, keep the detail out of the instruction lookup.
    return code.startsWith("hud_value_not_a_number:") ? "hud_value_not_a_number" : code;
  });

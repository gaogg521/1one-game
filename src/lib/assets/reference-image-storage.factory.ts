...
function normalizeMode(raw: string | undefined): ReferenceImageStorageMode {
  const v = (raw ?? "session").trim().toLowerCase();
  return v === "cloud" ? "cloud" : "session";
}

/** 服务端摄取 / 生成管线取存储适配器（单例，按 env 切换） */
let cached: IReferenceImageStorage | null = null;
let cachedKey = "";

export function getReferenceImageStorage(): IReferenceImageStorage {
  const key = `${process.env.REFERENCE_ASSET_STORAGE ?? "session"}`;
  if (cached && cachedKey === key) return cached;
  cachedKey = key;
  const mode = normalizeMode(process.env.REFERENCE_ASSET_STORAGE);

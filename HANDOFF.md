...
### 检测→修复闭环的生产实测（seq5 / seq6）

同一项目一次定向修改 + 一轮自动打磨后，与今早 seq2 对比：

| 项 | seq2 | seq5/6 |
|---|---|---|
| 屏幕填充 | 82%（上下 73px 黑边） | **100%**（540×1170 精确铺满 393×852） |
| 结算 | 时间归零不结算 | **`forge-end` 触发**，结算卡可用 |
| 主角美术 | `ship_blue.png` 404 | **200**（素材重试补回） |
| 主角配色 | 灰紫 `#9b8cb2` | 亮蓝 `#4fc3f7` |
| 页面报错 | 有 | 无 |
| 计分/扣血 | 不生效 | **仍不生效** |

`core_loop_unresolved` 在一个 `passed`/评分 98 的构建上正确触发并排入打磨轮 `round 2/3`；seq5 全程保持 ready 不受打磨影响。**检测与路由已验证，修复本身这一轮未成功。**

### 实测逼出的两个真 bug（已修）

- `443c962e`：**美术审查不是配置缺失，是超时太短。** 我加的日志直接推翻了自己的判断——`[visual-review] model=doubao-seed-2-1-turbo-260628 ok=false reason=...AbortSignal... ms=60002`。模型路由正常且可达，是被本平台自己的 AbortSignal 在 60 秒掐断。已改 120 秒。**不要再把它当成后台配置问题。**
- `a5f436e0`：**视觉合同的第二个假阳性。** 它查字面量 `ctx.assets.player`，而设计提示词要求模型按主题命名素材键（`ship_blue`/`meteor_red`），所以每个正确构建都被报"三个素材槽全未使用"——并且这三条假发现**进了打磨轮**，和唯一的真问题一起送给代码 Agent，稀释了那一轮修复。现在从设计声明的键去查。

### 仍未解决

- **碰撞判定**：收集不加分、陨石不扣血。这是当前唯一挡住"可玩"的问题。
- **HUD 语言回退**：seq5 的 HUD 是 `SCORE/LIVES/TIME`，结算卡却是中文。代码 Agent 提示词明确要求用玩家语言，但没有任何检测覆盖这条。
...
## 2026-09-12 补充 · 缺陷的统一形状，以及部署的两个真实缺陷

### 所有让生成游戏不可玩的缺陷都是同一个形状

僵尸生成后不动、豌豆射手不发射、碰撞遍历空组、分数只记进私有 `G` 对象、输入从不读取、HUD 显示 NaN——**全部是「系统写出来了、接上了、画出来了，但从来没真正做过事」**。没有一条会抛异常，所以结构检查全部通过，玩家拿到一个西洋景。

不要再用「逐个手写精确修复指令」的方式处理它们：那是人肉 QA，不是平台质量，而且会掩盖「平台自己发现不了」这个真正的结论。每发现一类，就补一条**能覆盖整类**的自动检查。

已补的通用检查（`8390d4b`）：`forge-player-evidence` 本来就每 250ms 上报每个绘制精灵的坐标，此前无人消费。现在敌人/子弹这类「存在意义就是移动」的实体，若整个探针窗口位置一次都没变过，记 `advisory:entities_never_move:<kind>` 并送修复轮。

### 部署脚本的两个真实缺陷（未修，故意）

1. **原地重建 `.next`**：每次 `deploy-prod-with-assets.py` 都有真实的全站 500 窗口（首页也 500，`/api/health` 仍 200）。
2. **会杀掉正在跑的生成任务**：服务重启时若有生成任务正在跑运行时探针，它会拿到 `runtime_verification_unavailable`，整个修订判 `failed`。本会话实测发生过一次（seq7）。创作者那一侧看到的是「生成失败」，而他的游戏其实没问题。

这两条应一起修：构建到临时目录再原子切换，并在重启前等待活动 `game_production` 任务排空（或让探针在连接失败时重试而不是判失败）。**本会话末尾故意没动**——在跑了二十次部署之后改部署脚本本身，风险高于收益。

回滚（`0f4903ca`）在 seq7 失败时正确生效，项目回到 seq6 并保持可玩，没有下线。

### 「实体不动」检查的生产闭环验证（seq8，已完成）

同一项目一轮定向修复后，交付证据：

```
motion:enemy:493          ← 僵尸在探针窗口内实际移动 493 虚拟像素
ended:true                ← 整局产生结算（此前一直 false）
advisory:core_loop_unresolved  ← 消失
screenFillPct:100 · 四个素材全部 200 · blockers=[]
```

新检查双向验证通过：静止构建上该报（数据源已确认），移动构建上不误报。

真机试玩（393×852）：HUD 显示「阳光 N」「**击退 1/12**」，种植→发射→击杀→计数→结算整条链路闭合。这一局 20 秒落败属正常游戏结果（脚本机器人按固定列点击，不会按行拦截），不是缺陷。

**这款游戏的最终状态**：竖屏 100% 铺满、草坪与角色美术真实、点击种植可用、阳光经济正常、豌豆命中扣血、击退计数递增、胜负均有结算卡。项目 `cmty62y9g0009hnvaqtjwi0be`，`pending_review` 未发布。

剩余：`undeclared_shared_state`（又一个孤儿共享状态，路由已正确指向产生方，是修复轮成功率问题）、`collectible_spawn_outside_viewport`。两者都是 advisory，不阻断交付。

...
func _try_build_at_slot(slot_i: int) -> void:
	...
	if _camera:
		GameJuice.burst(self, _camera.unproject_position(pos), GameSpecData.theme_color("playerColor", Color.GREEN), 12)
...
func _spawn_enemy_entity(enemy_id: String, def: Dictionary, hp: float, speed: float, reward: int) -> void:
	...
	vis.mesh = mesh
	var mat := StandardMaterial3D.new()
	mat.albedo_color = Color(0.92, 0.22, 0.22)
	vis.material_override = mat
...
func _damage_enemy(e: Dictionary, proj: Dictionary, now: float) -> void:
	...
	if float(e.get("hp", 0)) <= 0.0:
		...
		if node:
			GameAudio.play_bleep(GameBleeps.Kind.PICKUP)
			if _camera:
				GameJuice.burst(self, _camera.unproject_position(node.global_position), GameSpecData.theme_color("collectibleColor", Color.GOLD), 14 if is_tank else 8)
			if is_tank:

...
## 2026-08-30 — 首个可玩预览与完整质量生产分离

**背景**：clean-room 复测 Astrocade 公开游戏后，在生产创作台用同类机制生成，超过一分钟仍停在固定“1/3”。现有请求可能串联 Brief、多 Agent、critic、二次强化、专属代码运行时，整段模型预算最高 300 秒，进度却没有反映这些阶段。

**决策**：
- `/api/generate/stream` 的职责是尽快交付可保存的设计预览：一次 LLM draft，模型链预算 40 秒、HTTP 端到端硬截止 45 秒，随后只做确定性校验；截止时立即用 kernel fallback 交付，不再等待迟到的模型请求。
- Creative Brief、多 Agent、critic、二次强化、资产与专属运行时代码统一放到保存后的 durable `game_production`，不得阻塞创作台首屏。
- 复杂玩法预览不使用通用几何占位。预览只标记 agentic route，保存后由 worker 真实生成可执行模块，再进入试玩与发布门禁。
- 进度只显示“生成 1/2 → 检查 2/2 → 完成”，并明确告知超时会交付基础版本；禁止把未知的模型等待伪装成固定 1/3。
...
## 2026-08-30 — 显式玩法意图必须先于宽泛动作词路由

- “车辆并排、名次/末位淘汰、决赛/杯赛”等组合是强赛车意图，优先级必须高于“躲避”这类跨品类动作词。
- 生产链若把赛车误分到 `avoider`，后续代码 Agent 会同时收到弹幕模板和赛车机制合同，造成冲突、超时或不可玩的混合产物；因此先在共享模板定义与预选器修正，不对单个作品打补丁。
- 任一提示命中至少 3 个显式机制时，视为 `agentic_complex` 并强制 bespoke runtime；即使已有专用 Scene，也不得用通用模板冒充该组合玩法。`agenticPlayRoute=agentic` 时，没有模块源码一律视为没有 bespoke runtime。

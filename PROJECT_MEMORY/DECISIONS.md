...
- durable worker 的一次 HTTP 执行上限约 640 秒，因此专属运行时每次 job attempt 固定为 1 个模型、最多 1 轮 repair（单调用 75/55 秒）；失败由持久化任务按 attempt 重试。禁止在一个租约里串行穷举多模型多轮修复，导致任务超时但状态长期停在 2%。
